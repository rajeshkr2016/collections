...
if echo "$TGTHOST"|grep -q "[3-4]3[0-9][0-9]" && echo "$CHOST"|grep -q "[3-4]3[0-9][0-9]" ; then
DCNAME="qydc"
elif 
fi
...
PROP1STAT=$?
echo "===================================================================================================================="
echo -e "\nComparing tmp/${TGTHOST}/webs-mule-${DCNAME}-${ENV}.properties and tmp/${CHOST}/webs-mule-${DCNAME}-${CENV}.properties"
bash -c "sdiff -b -s -w 250 <(sort tmp/${TGTHOST}/webs-mule-${DCNAME}-${ENV}.properties) <(sort tmp/${CHOST}/webs-mule-${DCNAME}-${CENV}.properties)"
PROP2STAT=$?
echo "===================================================================================================================="
...
echo "No difference between compared hosts for files: tmp/${TGTHOST}/{webs-mule-global-${ENV}.properties,webs-mule-${DCNAME}-${ENV}.properties,webs-mule-${ENV}-password.properties} and tmp/${CHOST}/{webs-mule-global-${CENV}.properties,webs-mule-${DCNAME}-${CENV}.properties,webs-mule-${CENV}-password.properties}"
else
echo "Differences in conf files found: tmp/${TGTHOST}/{webs-mule-global-${ENV}.properties,webs-mule-${DCNAME}-${ENV}.properties,webs-mule-${ENV}-password.properties} and tmp/${CHOST}/{webs-mule-global-${CENV}.properties,webs-mule-${DCNAME}-${CENV}.properties,webs-mule-${CENV}-password.properties}"
fi
fi
