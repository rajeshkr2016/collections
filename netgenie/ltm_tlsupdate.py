...
#            r = requests.get("https://netgenie.corp.company.net/ltmapi/v1/virtualserver?vsName="+vip.rstrip(), headers=header)
            ...
            if status == 200:
                ...
                print("Exception for "+vip +":"+ chomp(str(e)))
                continue
#Function to create teh JSON data based on the VIP Type
def formatRequestData(vServer, profile_type, profile_name="Company-standard-TLS_1.2", parent_profile="Company-standard-TLS_1.2"):
    json_update = {
...
def getTLSProfile(vip_ip):
    ...
    if vServer_ip_first_oct == 10:
        tls_profile = "Company-standard-TLS_1.2"
    else:
        tls_profile = "Company-standard-TLS_1.2"
    return tls_profile
